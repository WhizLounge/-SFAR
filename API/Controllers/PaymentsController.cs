    ...
    : BaseApiController
{
    ...
    public async Task<ActionResult<BasketDto>> CreateOrUpdatePaymentIntent()
    {
        ...
        basket.PamymentIntentId ??= intent.Id;
        ...
        if (context.ChangeTracker.HasChanges())
        {
            ...
            if (!result) return BadRequest("Problems updating basket with intent");
        }
    ...
    public async Task<ActionResult> StripeWebhook()
    {
       ...
       {
        logger.LogError(ex, "Undexpected error");
        return StatusCode(StatusCodes.Status500InternalServerError, "Stripe webhook error");
       }
    }
    ...
    private async Task HandlePaymentIntentSucceeded(PaymentIntent intent)
    {
        ...
        var basket = await context.Baskets
            .FirstOrDefaultAsync(x => x.PamymentIntentId == intent.Id);
    ...
    private Event ConstructStripeEvent(string json)
    {
       try
       {
         return EventUtility.ConstructEvent(
            ...
            config["StripeSetting:WhSecretKey"]);
